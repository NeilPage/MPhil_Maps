...
# Simulations
dataset1 = MFDataset('/Users/ncp532/Documents/Some_scripts_/nc_Dynamic_Ocean/trac_avg.geosfp_2x25_Hg.2013*.nc') # Dynamic Ocean (Standard GEOS-Chem Run)
dataset2 = MFDataset('/Users/ncp532/Documents/Some_scripts_/nc_LPOLARBR/trac_avg.geosfp_2x25_Hg.2013*.nc') # LPOLARBR (us ethe parametrisation for Br concentrations in the polar boundary layer)
...
def plotkg(x, bar, tit):  # (x = the value to be plotted, bar = colorbar (Y/N), tit = title for map)
    ...
    #          resolution='c',projection='spstere')      # choose the resolution and projection type
    
    # SOUTH POLAR STEROGRAPHIC PROJECTION          
    m = Basemap(projection='spstere', boundinglat=-30, lon_0=90, resolution='c')          
    ...
    lons1 = [144.6832, 77.3423, 18.4897, 62.5227, 77.5803, 110.3136, 158.5609, 140.0007, 123.1956, 2.3206, -71.2512, 166.4006]    # [Cape Grim, Amsterdam Island, Cape Point, Mawson, Davis, Casey, Macquarie Island, Dumont d'Urville, Concordia, Troll, Bariloche, McMurdo]
    lats1 = [-40.6832, -37.4748, -34.3535, -67.3612, -68.3436, -66.1657, -54.2959, -66.3946, -75.0559, -72.0041, -41.7438, -77.5047]  # [Cape Grim, Amsterdam Island, Cape Point, Mawson, Davis, Casey, Macquarie Island, Dumont d'Urville, Concordia, Troll, Bariloche, McMurdo]
    x1,y1 = m(lons1, lats1)
    ...
    # OPTION 1: if you want plain location markers use this line
    #m.scatter(x2, y2, c='white', edgecolors='black', s=200) # define the location markers (color, edgecolor, size)
    
    # OPTION2: if you want the location markers to represent observed Hg concentrations use these two lines
    #obs = [0.909, 0.974, 0.808, 1.032, 1.092] # define the observed Hg concentration for each site
    obs = SIPEXII_Hg0
    m.scatter(x2, y2, c=obs, vmin=0.4, vmax=1.6, cmap=cm.plasma, s=10) # define the location markers (color=obs, you need to set vmin/vmax and the cmap to same values as the colorbar above)  
    
    # PLOT LABELS FOR THE LOCATION MARKERS
    labels = ['Cape Grim', 'Amsterdam \n Island', 'Cape Point', 'Mawson', 'Davis', 'Casey', 'Macquarie \n Island', 'Dumont d\'Urville', 'Concordia', 'Troll', 'Bariloche', 'McMurdo'] # text you want displayed
    x_offsets = [-400000, -900000, -900000, -2000000, -900000, -900000, -900000, -900000, -900000, -900000, -900000, -900000] # Adjust the label positions on the x-axis
    y_offsets = [350000,  -1000000, -700000, -700000, -700000, -700000, -1000000, -700000, -700000, -700000, -700000, -700000]    # Adjust the label positions on the y-axis
...
ax1 = plotkg(Hg0_a_Av[:,:], 'Y', "Simulated Hg$^0$ Concentration (Sept & Oct)") # Note that Hg0 is an array Hg0['time','lev','lat','lon']
